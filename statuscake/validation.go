...
func IsEmailAddress(i interface{}, k string) ([]string, []error) {
	...
	if v == "" {
		return nil, []error{fmt.Errorf("expected %q email address to not be empty, got %v", k, i)}
	}

	if _, err := mail.ParseAddress(v); err != nil {
		return nil, []error{fmt.Errorf("expected %q to be a valid email address, got %v: %+v", k, v, err)}
	}
...
func StringIsNumerical(i interface{}, k string) ([]string, []error) {
	...
	if v == "" {
		return nil, []error{fmt.Errorf("expected %q number to not be empty, got %v", k, i)}
	}

	if _, err := strconv.Atoi(v); err != nil {
		return nil, []error{fmt.Errorf("expected %q to be a valid number, got %v: %+v", k, v, err)}
	}
...
func Int32InSlice(valid []int32) schema.SchemaValidateFunc {
	return func(i interface{}, k string) ([]string, []error) {
		v, ok := i.(int)
		if !ok {
			return nil, []error{fmt.Errorf("expected type of %s to be integer", k)}
		}
		...
		return nil, []error{fmt.Errorf("expected %s to be one of %v, got %d", k, valid, v)}
	}
}
